...
def api_mt4_commands():
    ...
    if request.method == 'POST':
        ...
        try:
            # 尝试转换旧格式到新格式
            if data.get('action') == 'OPEN' and data.get('order_type') == 'MARKET':
                # 转换旧格式的OPEN命令到新格式的MARKET命令
                converted_data = {
                    'account': data.get('account', 833711),  # 默认账户ID
                    'action': 'MARKET',
                    'symbol': data.get('symbol'),
                    'side': data.get('side'),
                    'risk_alloc_pct': (data.get('position_pct', 0) or 0) / 100.0,
                    'max_spread_points': int(data.get('max_slippage', 2) or 2),
                    'sl_points': int(data.get('sl', 0) or 0) if data.get('sl') and data.get('sl') < 1000 else 0,
                    'tp_points': int(data.get('tp', 0) or 0) if data.get('tp') and data.get('tp') < 1000 else 0
                }
                command_queue.add_command(converted_data)
        except Exception as e:
            # 如果转换失败，只记录日志，不影响数据库写入
            print(f"警告：无法将命令添加到内存队列: {e}")
        
        return jsonify({'status': 'success', 'command_id': cmd_id})

    # GET: MT4 EA 拉取一条待执行指令（旧系统，从数据库读取）
    row = query_db(
        ...
        <script>
            ...
            function submitOrder(side) {
                ...
                if (p.order_type === 'MARKET') {
                    // 市价单：使用 MARKET action
                    // 假设SL/TP输入的是点数（如果值很大，可能是价格，需要转换，这里简化处理）
                    let sl_points = 0, tp_points = 0;
                    if (p.sl > 0) {
                        // 如果值小于1000，假设是点数；否则假设是价格（需要根据品种转换）
                        if (p.sl < 1000) {
                            sl_points = Math.round(p.sl);
                        } else {
                            // 价格形式，需要转换为点数（简化：假设是标准外汇，1点=0.0001）
                            // 注意：这里需要根据实际品种调整
                            sl_points = Math.round((p.sl - 1.0) * 10000);  // 简化处理
                        }
                    }
                    if (p.tp > 0) {
                        if (p.tp < 1000) {
                            tp_points = Math.round(p.tp);
                        } else {
                            tp_points = Math.round((p.tp - 1.0) * 10000);  // 简化处理
                        }
                    }
                    
                    payload = {
                        account: 833711,  // 从MT4状态上报中获取，这里先硬编码
                        action: 'MARKET',
                        symbol: p.symbol,
                        side: p.side,
                        risk_alloc_pct: p.position_pct / 100.0,  // 转换为小数
                        max_spread_points: Math.round(p.max_slippage || 2),
                        sl_points: sl_points,
                        tp_points: tp_points
                    };
                } else {
                    ...
                    payload = {
                        ...
                        symbol: p.symbol,
                        side: p.side,
                        volume: p.position_pct / 100.0,  // 简化处理，实际应该根据资金计算
                        price: p.limit_price,
                        sl: p.sl > 0 ? p.sl : 0,
                        tp: p.tp > 0 ? p.tp : 0,
                        max_spread_points: Math.round(p.max_slippage || 2)
                ...
                .then(d => {
                    if (d.status === 'success') {
                        alert('已发送至 MT4 指令队列，Command ID: ' + d.cmd_id);
                    } else {
...
def receive_snapshot():
    ...
    try:
        ...
        # 保存到数据库（可选：如果需要持久化快照数据，可以创建相应的表）
        # 当前实现：仅记录日志，不持久化（因为状态上报已经包含持仓信息）
        print(f"收到快照: account={account}, positions={len(positions)}, history={len(history)}")
