...
bool ExecuteMarket(JSONObject &cmd, int &ticket, string &error_code, string &error_msg, double &out_spread){
   ...
   if(lot_step > 0) volume = MathFloor(volume / lot_step) * lot_step;
   if(volume < min_lot) volume = min_lot; // 至少最小手
   if(volume > max_lot) volume = max_lot; // 不超最大手
   
   // 2. 保证金检查
   int op_type = (side=="buy") ? OP_BUY : OP_SELL;
   double free_margin = AccountFreeMarginCheck(symbol, op_type, volume);
   
   Print("=== Pre-Check: Symbol=", symbol, " RawLots=", raw_volume, " NormLots=", volume, 
         " FreeMarginCheck=", free_margin, " Err=", GetLastError());
   
   if(GetLastError() == 134 || free_margin <= 0) { // ERR_NOT_ENOUGH_MONEY
       error_code = "not_enough_money";
       error_msg = "资金不足(预检): Required > FreeMargin";
       return false;
   }
...
bool ExecuteLimit(JSONObject &cmd, int &ticket, string &error_code, string &error_msg, double &out_spread){
   ...
   // 挂单保证金预检 (Pending order margin check is tricky, using current price approximation)
   int op_type = (side=="buy") ? OP_BUYLIMIT : OP_SELLLIMIT; // 简化假设 limit
   if (price > MarketInfo(symbol, MODE_ASK) && side=="buy") op_type = OP_BUYSTOP;
   if (price < MarketInfo(symbol, MODE_BID) && side=="sell") op_type = OP_SELLSTOP;

   double free_margin = AccountFreeMarginCheck(symbol, op_type, volume);
   
   Print("=== Pre-Check (Limit): Symbol=", symbol, " RawLots=", raw_volume, " NormLots=", volume, 
         " FreeMarginCheck=", free_margin);
   
   if(GetLastError() == 134 || free_margin <= 0) { 
       error_code = "not_enough_money";
       error_msg = "资金不足(预检-挂单)";
       return false;
   }
...
void ProcessCommand(JSONObject &cmd){
   ...
   // 移除严格的时间差检查
   // 由于 EA (TimeCurrent) 和 Flask (time.time()) 可能存在时区或时钟差异，
   // 直接比较可能导致命令被误判为 expired。
   // 实际上 Flask 端已经有 cleanup_expired_commands 逻辑，过期的命令根本不会下发给 EA。
   // 因此，EA 端只负责执行，不再做严格的 TTL 校验，除非时间差极其离谱（防止重放攻击）
   
   if(created_at > 0){
      // 仅当时间差超过 24 小时才视为异常，防止完全失效的历史命令
      // 允许 24 小时的时间偏差，足以覆盖所有时区差异
      if(MathAbs(time_diff) > 86400){
         Print("=== DEBUG: timestamp anomaly rejected (diff > 24h) ===");
         SendReport(id,nonce,false,0,"expired","命令时间戳异常(>24h)");
         return;
      }
   }
